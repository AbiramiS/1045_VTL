	    ...
	    public void onClick( View view ) { 
	    	...
	    	try{  
	    		...
	    		if(done) {
					...
					HttpGet getRequest=new HttpGet(getResources().getString(R.string.server_url)+pref.getString("serverip", "000000000000000")+getResources().getString(R.string.server_url1)+"userregistration.do?username="+username.getText().toString()+"&password="+password.getText().toString()+"&gender="+rb.getText().toString().trim()+"&mobileno="+mobno.getText().toString().trim()+"&emailid="+emailid.getText().toString().trim());
					try {    
				        ...
				        if(val1.equals("ALREADY EXISTING")){
				        	Toast.makeText(getApplicationContext(), "User Details Already Existing", Toast.LENGTH_LONG).show(); 
				        	startActivity(new Intent(getApplicationContext(), RegFailure.class));
				        }
				        else if(val1.equals("REGISTRATION SUCCESSFULL")){
				        	Toast.makeText(getApplicationContext(), "Your User Id. is: "+val2, Toast.LENGTH_LONG).show(); 
